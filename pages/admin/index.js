...
function useApi401Redirect() {
  ...
  const gotoLogin = () => {
    ...
    // تحويل صلب عبر المتصفح يمنع إلغاء تحميل مكوّنات Next
...
function SettingsTab({ api }) {
  ...
  async function load() {
    ...
    setForm({
      ...
      orgLogo: resolveSrc(s.orgLogo||''),
      eventLogo: resolveSrc(s.eventLogo||''),
      bannerEnabled: !!s.bannerEnabled,
      bannerImage: resolveSrc(s.bannerImage||''),
    ...
    <div className="card">
      ...
      <div className="grid2">
        <div>
          ...
          {form.orgLogo && <img className="preview" src={resolveSrc(form.orgLogo)} alt="org" />}
        </div>
        <div>
          ...
          {form.eventLogo && <img className="preview" src={resolveSrc(form.eventLogo)} alt="event" />}
        </div>
      </div>
      ...
      {form.bannerImage && <img className="preview" src={resolveSrc(form.bannerImage)} alt="banner" />}
...
/* ===================== تبويب: المتحدثون ===================== */
// القراءة من /api/speakers (عام)، والكتابة إلى /api/admin/speakers (محمي)
function SpeakersTab({ api }) {
  ...
  function startEdit(row){ setEditingId(row._id); setForm({
    name:row.name||'', title:row.title||'', talk:row.talk||'', bio:row.bio||'', photo:resolveSrc(row.photo||'')
  }) }
  ...
  return (
    <>
      <div className="card">
        ...
        {form.photo && <img className="preview" src={resolveSrc(form.photo)} alt="speaker" />}
        ...
        {rows.map(r=>(
          <div key={r._id} className="card">
            <div className="row" style={{gap:12,alignItems:'center'}}>
              {r.photo && <img src={resolveSrc(r.photo)} alt={r.name} style={{width:64,height:64,objectFit:'cover',borderRadius:8,border:'1px solid #eee'}}/>}
              <div>
  ...
  return (
    <>
      <div className="card">
        ...
        {form.logo && <img className="preview" src={resolveSrc(form.logo)} alt="logo" />}
        ...
        {rows.map(r=>(
          <div key={r._id} className="card" style={{textAlign:'center'}}>
            <div className="logo-wrap">
              {r.logo ? <img src={resolveSrc(r.logo)} alt={r.name} /> : <div className="logo-ph">{r.name}</div>}
            </div>
...
function AdminPage(){
  ...
  const initOnceRef = useRef(false) // يمنع تهيئة التبويب أكثر من مرة

  // حارس الجلسة (تشغيل مرة واحدة فقط)
  useEffect(() => {
  ...
  // ضبط التبويب بعد اكتمال التحقق (مرة واحدة فقط مع منع replaceState المتكرر)
  useEffect(() => {
  ...
  return (
    <main dir="rtl" className="page">
      <div className="container">
        ...
        {tab==='speakers'  && <SpeakersTab api={api}/>}
